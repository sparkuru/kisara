...
`config/features/setu/config.toml` 启用后，归档功能仅处理同时位于全局白名单和功能白名单中的 OneBot 私聊用户。配置独立于接入凭据与其他功能；缺少该文件时归档功能关闭。普通图片和直接发送的合并转发不触发归档回复。用户引用合并转发并发送 `保存`、`setu` 或 `/setu` 后，该转发形成一个批次，立即生成确认提示；用户须引用该提示并回复“保存”，默认 60s 后自动取消，不再处理取消词。引用合并转发并发送 `direct_confirm_words` 中的词（默认 `直接保存`）时跳过询问，直接保存并返回保存结果；部分失败时可引用结果消息重试失败项。`confirm_words` 和 `confirm_timeout_seconds` 可配置确认词及超时，旧 `cancel_words` 字段只兼容读取，不再生效。合并转发递归展开设有深度和节点上限。白名单用户引用图片并发送 `/export-img`、`导出` 或 `转图片` 时，导出工作流将原始图片逐张作为文件附件发送，保留原始字节及动画，避免 QQ 将再次发送的图片归类为不可保存的表情；此操作优先于归档且不依赖归档配置。

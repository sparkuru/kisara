#!/usr/bin/env bash
# preview.sh - Start and stop the selected Kisara engine.
#
...
SCOPE_LABEL="hako.scope=preview.sh"
...
down_services() {
	...
	if [[ -z "${container_ids_text}" ]]; then
		[[ "${mode}" == quiet ]] || printf '[preview.sh] no dev containers found\n' >&2
		return 0
	fi

	mapfile -t container_ids <<<"${container_ids_text}"
	printf '[preview.sh] stopping %s bot container(s)\n' "${#container_ids[@]}" >&2
...
run_service() {
	...
	printf '[preview.sh] starting %s through ./hako\n' "${name}" >&2
	HAKO_SCOPE=preview.sh \
		HAKO_SERVICE="${name}" \
...
start_services() {
	...
	*)
	...
	printf '[preview.sh] selected engine: %s; no host port is published\n' "${engine}" >&2
